"""
清理 7 天前的 holdings_data 檔案（以台灣時間 UTC+8 計算 cutoff，避免在 GitHub Actions UTC 環境誤刬）
...
# 與 main / scraper 一致：以台灣日曆日判斷「超過 N 天」（GitHub Actions 預設為 UTC，避免誤刬）
...
def cleanup_old_holdings(days=7, dry_run=False):
    ...
    cutoff = _now_taiwan() - timedelta(days=days)
    ...
    for path in files:
        ...
        if m:
            try:
                file_time = datetime(
                    int(m.group(1)), int(m.group(2)), int(m.group(3)), tzinfo=TAIWAN_TZ
                )
                if file_time < cutoff:
                    if not dry_run:
                        os.remove(path)
            ...
            try:
                ...
                file_time = datetime.fromtimestamp(mtime, tz=timezone.utc).astimezone(TAIWAN_TZ)
                if file_time < cutoff:
                    if not dry_run:
                        os.remove(path)
                    deleted.append(name)
