...
- **自動執行時間**：每天台灣時間 **14:30**（同一個 workflow 內依序：抓取 → 提交 JSON → 發送 Telegram）
- **時區換算**：台灣 14:30 = UTC **06:30**（cron：`30 6 * * *`；較原 16:30 提早 2 小時，預留排程排隊延遲）
...
4. **時區**：
   ...
   - 目前排程為台灣時間 **14:30**（抓取與發送同一 workflow）
